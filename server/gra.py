...
def pobierz_hasla(kategoria):
    ...
    if kategoria == "MIX":
        mycursor.execute("SELECT tresc FROM hasla")
    else:
...
class Gra(object):
    ...
    def __init__(self, id, gracze, kategoria):

        self.id = id
        self.gracze = gracze

        self.hasla = pobierz_hasla(kategoria)  # pobranie hasel z bazy danych
    ...
    def nowa_runda(self):
        # Przy każdej rundzie wartość licznika rund oraz indeks gracza rysującego zostanie zinkrementowany,
        # dzięki czemu będzie wiadomo kiedy odbyły sie wszystkie kolejki i należy już zakończyć grę
        try:
    ...
    def get_haslo(self):
        ...
        while haslo in self.uzyte_hasla:
            hasla.remove(haslo)
            haslo = random.choice(hasla)
